...
namespace histo
{
    ...
    void Histogram::calculatehistogram(dtype *data, int shape[3], int n_bins, dtype lowerbound, dtype upperbound, std::vector<uint64_t> &out_histobins, std::vector<double> &out_histoedges)
    {
        ...
        for (uint64_t idx = 0; idx < nstack; idx++)
        {
            ...
            if((datavalue < lowerbound) || (datavalue > upperbound))
                continue;
            if (ignore_zero && datavalue == 0.0f) continue;
    ...
    void Histogram::calculatehistogram(dtype *data, uint8_t *mask, int shape[3], int n_bins, dtype lowerbound, dtype upperbound, std::vector<double> &out_histobins, std::vector<double> &out_histoedges)
	{
		long long int nslice = shape[0]*shape[1];
		long long int nstack = nslice*shape[2];
    ...
    std::vector<double> Histogram::normalize(std::vector<uint64_t> histobins, std::string normalization)
    {
        ...
        else if (normalization == "mean&std")
        {
    		double stdev = 0.0;

    		for (int i = 0; i < histobins.size(); i++)
    		{
    			double val = histobins[i];
    			mean += val;
    			stdev += val*val;
    		}
    		mean /= histobins.size();
    		stdev = std::sqrt(stdev / (histobins.size() - 1.) - (mean * mean * histobins.size()) / (histobins.size() - 1.));
    		weight = stdev;
        }

        for(uint64_t idx; idx < histobins.size(); idx++)
            output[idx] = ((double) (histobins[idx]-mean))/weight;
    ...
    float* Histogram::calculate2DHistogram(float *frame0, float *frame1, uint8_t *mask, int shape[3], std::vector<double> &histoedges0, std::vector<double> &histoedges1, int outshape[2])
	{
		long long int nslice = shape[0]*shape[1];
		long long int nstack = shape[2]*nslice;
		long long int nrange = 0;
