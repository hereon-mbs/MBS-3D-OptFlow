...
namespace hdcom
{
    ...
    int HdCommunication::makepath(std::string s)
    {
        size_t pos=0;
        std::string dir;
        int mdret = -1;
    ...
    uint8_t* HdCommunication::Get2DTifImage_8bitPointer(std::string file, int outshape[2])
	{
		char *path;
		path = new char[file.length()+1];
		strcpy(path, file.c_str());
    ...
    float* HdCommunication::Get3DTifSequence_32bitPointer(std::vector<std::string> &filelist, int outshape[3], std::pair<int,int> slice_range)
	{
		float* imgstack;

		int firstslice = slice_range.first;
		int lastslice = slice_range.second-1;
